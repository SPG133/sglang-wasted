...
class Req(ReqDllmMixin):
    ...
    def record_execution_start(
        ...
        # 一个 request 会被切成多轮 prefill/decode step，每轮单独压栈，结束时累加。
    ...
    def finalize_scheduler_timing(self, timestamp: Optional[float] = None):
        # 等待时间 = 从入队到结束的总时间 - prefill/decode 实际执行时间 - KV 传输时间。
        end_time = timestamp if timestamp is not None else time.monotonic()
        self.actual_execution_time = (
            self.prefill_execution_time + self.decode_execution_time
        )
    ...
    def assign_mlfq_level_for_next_iter(self, next_iter_tokens: int):
        # Skip-join：新请求不都进 Q1，而是按下一轮预计 token 数直接进入合适队列。
    ...
    def update_mlfq_after_schedule(self, scheduled_tokens: int):
        # 量子耗尽则降级；decode 每步通常 scheduled_tokens=1，长任务会逐步下沉。
        mlfq_quanta = (1, 2, 4)
...
class ScheduleBatch(ScheduleBatchDisaggregationDecodeMixin):
    ...
    def retract_decode(
        ...
        if not server_args.speculative_algorithm:
            if server_args.schedule_policy == "mlfq":
                # MLFQ 感知的显存丢弃：OOM 时优先 retract 低优先级队列里的请求。
                # sorted_indices 后面用 pop() 取牺牲者，因此优先保留的请求排在前面。
                sorted_indices.sort(
                    key=lambda i: (
                        getattr(self.reqs[i], "mlfq_level", 0),
                        getattr(self.reqs[i], "mlfq_tokens_in_level", 0),
